...
(defun resolve-on (lambda K1 K2)
	(when (and lambda k1 k2)
		(when
			(or (and (extract-positive-clauses lambda (list k1))   	;; Si la primera clausula tiene literales 'lambda' positivos
						(extract-negative-clauses lambda (list k2)))     	;; y la segunda 'lambda' negativos, devuelve la union de ambas sin estos literales
					(and (extract-positive-clauses lambda (list k2))		;; Si la primera clausula tiene literales 'lambda' negativos
						(extract-negative-clauses lambda (list k1))))			;; y la segunda 'lambda' positivos, devuelve la union de ambas sin estos literales
				(extract-literals lambda k1 k2))))

(defun extract-literals (lambda k1 k2)												;; Devuelve la union de las listas sin los literales 'lambda'
	(list (extract-literal lambda (union k1 k2))))

(defun extract-literal (elt lst)															;; Comprueba si existe 'elt' en la lista y devuelve la misma sin copias de 'elt'
	(when lst
